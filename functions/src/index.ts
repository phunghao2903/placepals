...
export const sendWelcomePushOnLogin = onCall(async (request) => {
  ...
  const registeredToken = asNonEmptyString(userSnapshot.data()?.androidFcmToken);
  ...
  try {
    ...
    logger.error('Failed to send welcome push.', error);
